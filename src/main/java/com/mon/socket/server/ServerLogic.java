...
public class ServerLogic
{
    ...
    )
    {
        return session
                ...
                .doOnNext(message -> logger.info(
                        "Server -> received from client id=[{}]: [{}]",
                        session.getId(),
                        message
                ))
                .filter(message -> newClient.get())
                .flatMap(message -> sendAtInterval(
                        session,
                        interval
                ))
    ...
    private Flux<Void> sendAtInterval(
            WebSocketSession session,
            long interval
    )
    {
